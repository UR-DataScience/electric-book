...
* Add a `translations` node to the `meta.yml` with `directory` and `language`. They can optionally include their own work-level metadata such as `title`. Any metadata not added to a translation will be inherited from the original language. See [Metadata](translations.md#metadata) below.
...
If you add a stylesheet to a translation's `styles` directory \(e.g. `book/fr/styles/print-pdf.scss`\), those styles are _added to_ the parent CSS. Your pages will load the parent CSS, followed by your translation CSS.
...
`tag once earlier in each markdown file to load the`\` tag, along with a range of other useful tags.
...
>
> If that page exists, and a user navigates to a landing page containing a query string defining the language, they will be redirected to the actual, fully translated landing page for that language. {:.box}
